...
async def get_aggregated_template_fields(bank_code: str, template_id: str) -> JSONResponse:
    ...
    try:
        ...
        async with MultiDatabaseSession() as db:
            # Get the single unified banks document
            ...
            # Find the specific bank within the unified document
            target_bank = None
            ...
            # Find the specific template within the target bank
            ...
            # For the simple consolidated structure, we'll create a simplified response
            # Get common form fields (these remain the same)
            ...
            for doc in common_fields_docs:
                # Process group fields in this document
                processed_fields = process_group_fields(doc.get("fields", []))
                common_fields.extend(processed_fields)
            
            # Process the template fields (simple field names)
            ...
            # Create bank-specific tabs structure
            bank_specific_tabs = [
                {
        ...
        logger.error(f"❌ Error in single document aggregation: {str(e)}")
        import traceback
        traceback.print_exc()
