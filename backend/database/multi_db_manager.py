...
class MultiDatabaseManager:
    ...
    async def connect(self) -> bool:
        ...
        for attempt in range(max_retries):
            try:
                logger.info("Connecting to MongoDB Atlas (Multi-Database) - Attempt %s/%s...", attempt + 1, max_retries)
                ...
                for db_type, db_name in self.database_names.items():
                    ...
                    logger.info("📋 Connected to %s database: %s", db_type, db_name)
                ...
                for db_type, db_name in self.legacy_databases.items():
                    self.databases[db_type] = self.client[db_name]
                    logger.info("🔄 Connected to legacy %s database: %s", db_type, db_name)
                self.is_connected = True
                logger.info("✅ Successfully connected to all MongoDB Atlas databases")

                return True
            except (ConnectionFailure, ServerSelectionTimeoutError) as e:
                logger.warning("⚠️ Connection attempt %s failed: %s", attempt + 1, e)
                ...
                logger.error("💥 Unexpected error during MongoDB connection: %s", e)
                self.is_connected = False
                if attempt < max_retries - 1:
                    logger.info("🔄 Retrying in %s seconds...", retry_delay)
                    await asyncio.sleep(retry_delay)
                    retry_delay *= 2
        logger.error("❌ Failed to connect to MongoDB Atlas after %s attempts", max_retries)
        return False
    async def ensure_connection(self) -> bool:
        ...
        try:
            ...
            logger.warning("⚠️ Connection health check failed: %s", e)
            self.is_connected = False
            return await self.connect()
    ...
    def get_database(self, db_type: DatabaseType) -> AsyncIOMotorDatabase[Any]:
        ...
        database = self.databases.get(db_type)
        if database is None:
            raise ValueError(f"Database {db_type} not found")
        return database
    def get_shared_database(self) -> AsyncIOMotorDatabase[Any]:
        ...
        if not self.is_connected:
            raise RuntimeError("Database not connected. Call connect() first.")
        shared_db = self.databases.get("shared")
        if shared_db is not None:
            return shared_db
        admin_db = self.databases.get("admin")
        if admin_db is None:
            raise RuntimeError("Neither shared nor admin database available")
        return admin_db
    ...
    def get_org_database(self, org_id: str) -> AsyncIOMotorDatabase[Any]:
        ...
        if org_id in self.org_database_cache:
            logger.debug("📦 Using cached database for org: %s", org_id)
        ...
        logger.info("🆕 Loaded new organization database: %s", org_id)
        ...
        if len(self.org_database_cache) > self.max_org_cache_size:
            ...
            logger.debug("🗑️ Evicted %s from org database cache", oldest_org)
    ...
    async def ensure_org_database_structure(self, org_id: str) -> bool:
        ...
        logger.info("🏗️ Ensuring database structure for organization: %s", org_id)
        try:
            org_db = self.get_org_database(org_id)
            ...
            for collection_name, description in standard_collections.items():
                if collection_name not in existing_collections:
                    ...
                    logger.info("✅ Created collection: %s.%s - %s", org_id, collection_name, description)
                else:
                    logger.debug("✓ Collection already exists: %s.%s", org_id, collection_name)
            logger.info("✅ Database structure verified for organization: %s", org_id)
            return True
        except Exception as e:  # pylint: disable=broad-exception-caught
            logger.error("❌ Failed to ensure database structure for %s: %s", org_id, e)
            return False
    # ================================
    ...
    async def insert_one(self, db_type: DatabaseType, collection_name: str, document: Dict[str, Any]) -> ObjectId:
        ...
        logger.info("📝 Inserted document into %s.%s: %s", db_type, collection_name, result.inserted_id)
    ...
    async def update_one(self, db_type: DatabaseType, collection_name: str, filter_dict: Dict[str, Any], 
        ...
        if not current_doc:
            logger.warning("Document not found for update in %s.%s", db_type, collection_name)
            return False
        ...
        logger.info("📝 Updated document in %s.%s (new version: %s)", db_type, collection_name, result.inserted_id)
        return True
    ...
    async def delete_one(self, db_type: DatabaseType, collection_name: str, filter_dict: Dict[str, Any], 
        ...
        if success:
            logger.info("🗑️ Soft deleted document from %s.%s", db_type, collection_name)
        else:
            logger.warning("No document found to delete in %s.%s", db_type, collection_name)
