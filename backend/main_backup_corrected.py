...
try:
    ...
    # Fallback to simpler bcrypt setup is handled by verify_password function
...
async def transform_flat_to_template_structure(
    ...
    """
    Transform form data to hierarchical template structure matching SBI template design
    ...
    """
    try:
        logger.info(f"🔄 Starting transformation of form data to hierarchical template structure")
        ...
        FIELD_TO_LOCATION = {
            ...
            "property_location": {"tab": "Property Details", "section": "Section 2", "type": "textarea"},
            "city_town_village": {"tab": "Property Details", "section": "Section 2", "type": "textarea"},
        ...
        # First, check if the data is already in hierarchical format
        if isinstance(input_data, dict):
            expected_tabs = ['Property Details', 'Site Characteristics', 'Valuation', 'Construction Specifications', 'Detailed Valuation', '_common_fields_']
            has_hierarchical_format = any(tab in input_data for tab in expected_tabs)
            
            if has_hierarchical_format:
                logger.info(f"✅ Data already in hierarchical template format, returning as-is")
                return input_data
        ...
        def flatten_nested_data(data, prefix=""):
            ...
            for key, value in data.items():
                ...
                if isinstance(value, dict):
                    # Check if this dict contains actual form field values (not sub-objects)
                    has_non_dict_values = any(not isinstance(v, dict) for v in value.values())
                    
                    if has_non_dict_values:
                        # This level contains actual field values, extract them
                        for sub_key, sub_value in value.items():
                            if not isinstance(sub_value, dict):
                                # This is an actual field value
                                result[sub_key] = sub_value
                            else:
                                # Recurse deeper
                                result.update(flatten_nested_data(sub_value, sub_key))
                    else:
                        # All values are dicts, recurse deeper
                        result.update(flatten_nested_data(value, new_key))
                else:
        ...
        flat_data = flatten_nested_data(input_data)
        logger.info(f"� Flattened {len(input_data)} top-level keys to {len(flat_data)} flat fields")
        ...
        logger.info(f"✅ Hierarchical transformation complete:")
        logger.info(f"   📂 {len(result)} tabs created")
        for tab_name, tab_data in result.items():
            if tab_name == '_unmapped_':
                logger.info(f"   ⚠️ {tab_name}: {len(tab_data)} unmapped fields")
            elif isinstance(tab_data, dict):
                sections = sum(1 for v in tab_data.values() if isinstance(v, dict))
                direct_fields = sum(1 for v in tab_data.values() if not isinstance(v, dict))
                logger.info(f"   📂 {tab_name}: {sections} sections, {direct_fields} direct fields")
        
        return result
        
    except Exception as e:
        logger.error(f"❌ Error in hierarchical template transformation: {e}")
        logger.error(f"❌ Exception type: {type(e)}")
        logger.error(f"❌ Exception args: {e.args}")
        import traceback
        logger.error(f"❌ Full traceback: {traceback.format_exc()}")
        # Return flat data as fallback
        flat_data = flatten_nested_data(input_data) if 'flatten_nested_data' in locals() else input_data
        return flat_data
    ...
    try:
        ...
        await log_activity(
            ...
            details={
                ...
                "property_address": report_request.property_address,
                "status": "draft",
                "data_organization": "tab_based"  # NEW: Indicate new structure
            },
            ip_address=get_client_ip(request)
        )
...
async def create_custom_template(
    ...
    try:
        ...
        # For system admin, use the organization they're currently working in
        # For regular users, use their own organization
        if org_context.is_system_admin:
            # Extract org from URL or use org_short_name from context
            current_org = org_context.org_short_name
            org_db = db_manager.get_org_database(current_org)
            target_org_id = current_org
        else:
